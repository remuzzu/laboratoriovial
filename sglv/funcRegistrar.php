...
if (!empty($_POST)) {
	...
	if (isNull_registro($nombre, $email, $password, $con_password)) {
	...
	if (validaUsuarioImae($email) == 0) {
		$errors[] = "El usuario no corresponde a una persona del laboratorio vial";
	}
		...
		if ($arr['success'])	//Si es TRUE significa que nuestro captcha se validó correctamente
		{
			...
			if ($registro > 0) {
				...
				$url = 'http://' . $_SERVER['SERVER_NAME'] .
					'/laboratoriovial/sglv/activar.php?id=' . $registro . '&val=' . $token;
				$asunto = 'Activar Cuenta - Sistema de Usuarios';
				$cuerpo = mensajeActivacion($url, true);
